...
<form id="formA" name="formA" method="post" class="validable" 
      action="<?php echo $PATH_DOMAIN ?>/buscarArchivoPrestamo/<?php echo $PATH_EVENT ?>/">
...
var sessioncant="";
    $("#flex1").flexigrid
    ({
        url: '<?php echo $PATH_DOMAIN ?>/buscarArchivoPrestamo/search/',
        dataType: 'json',
        colModel : [
...
$("#archivos").val($('.trSelected div',grid).html());
        if (com=='Imprimir'){            
            $("#formA").attr("action","<?php echo $PATH_DOMAIN ?>/buscarArchivoPrestamo/rpteBuscar/");
            document.getElementById('formA').submit();                        
        }
if (com=='Adicionar'){  
...
if(cadena==""){
   ...
   document.getElementById("archivos").value+=",";  
    ...
    t++;    
      ...
      var url="<?php echo $PATH_DOMAIN."/prestamos/recarga/";?>";
        ...
        if (com=='Exportar'){
            if($('.fil_chk',grid).length > 0){
                ...
                //if($("#sw").val()=='1'){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<?php echo $PATH_DOMAIN ?>/buscarArchivoPrestamo/<?php echo $PATH_EVENT_EXPORT ?>/",
...
<script type="text/javascript">
    ...
    $("#fon_id").change(function(){
        if($("#fon_id").val()==""){
            ...
            $.ajax({
                url: '<?php echo $PATH_DOMAIN ?>/buscarArchivoPrestamo/loadAjaxUnidades/',
                type: 'POST',
    ...
    $("#uni_id").change(function(){  
        if($("#uni_id").val()==""){
            ...
            $.ajax({
                url: '<?php echo $PATH_DOMAIN ?>/buscarArchivoPrestamo/loadAjaxSeries/',
                type: 'POST',
    ...
    $("#ser_id").change(function(){  
        if($("#ser_id").val()==""){
            ...
            $.ajax({
                url: '<?php echo $PATH_DOMAIN ?>/buscarArchivoPrestamo/loadAjaxTramites/',
                type: 'POST',
   ...
   $("#tra_id").change(function(){  
        if($("#tra_id").val()==""){
            ...
            $.ajax({
                url: '<?php echo $PATH_DOMAIN ?>/buscarArchivoPrestamo/loadAjaxCuerpos/',
                type: 'POST',
