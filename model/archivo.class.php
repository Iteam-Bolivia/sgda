...
class archivo extends tab_archivo {
    ...
    // Busqueda principal
    function buscarNuevo($busquedaArray) {
         $valor="";$id_listar="";
        if(isset($_SESSION['id_lista'])){
          $id_listar=$_SESSION['id_lista'];
        }
        ...
        $sort = " ORDER BY tab_expediente.exp_id ";
        
        $sql = "$select $from $where $sort $limit";
        $result2 = $tarchivo->dbSelectBySQL($sql); //print $sql;
        $sql_c = "SELECT COUNT(tab_expediente.exp_id) $from $where ";
        $total = $tarchivo->countBySQL($sql_c);
        ...
        foreach ($result2 as $un2) {   
            ...
            //$json .= "cell:['" . $un->fil_id . "'";
            $json .= "cell:[";
            $json .= "'<input id=\"chkid_" . $un2->exp_id . "\" restric=\"" . " " . "\" class=\"fil_chk".$j."\" type=\"checkbox\" value=\"" . $un->exp_id . "\" />'";
            
            $json .= ",'<img src=\"" . PATH_DOMAIN . "/web/lib/32/document-png\" file=\"$un2->exp_id\" valueId=\"" . $un2->exp_id . "\" restric=\"" . " " . "\" class=\"viewFile icon\" />'";

            $json .= ",'<img src=\"" . PATH_DOMAIN . "/web/lib/32/b_view.png\" file=\"$un2->exp_id\" valueId=\"" . $un->exp_id . "\" restric=\"" . "" . "\" class=\"view icon\" />'";
            
            $json .= ",'" . addslashes($un2->fon_cod . DELIMITER . $un2->uni_cod . DELIMITER . $un2->tco_codigo . DELIMITER . $un2->ser_codigo . DELIMITER . $un2->exp_codigo) . "'";            
            //$json .= ",'" . $un->fil_id . "'";                        
            //$json .= ",'" . addslashes($un->fon_codigo) . "'";
            $json .= ",'" . addslashes(utf8_decode($un2->uni_descripcion)) . "'";
            //$json .= ",'" . addslashes(utf8_decode($un->ser_categoria)) . "'";
            $json .= ",'" . addslashes(utf8_decode($un2->exp_titulo)) . "'";
            //$json .= ",'" . addslashes(utf8_decode($un->cue_descripcion)) . "'";                        
            $json .= ",'" . addslashes(utf8_decode("")) . "'"; // titulo documento
            $json .= ",'" . addslashes(utf8_decode("$un2->exp_nomprod")) . "'";
            //$json .= ",'" . addslashes(utf8_decode($un->fil_firma)) . "'";
            //$json .= ",'" . addslashes(utf8_decode($un->fil_cargo)) . "'";
            //$json .= ",'" . addslashes($un->fil_nrofoj) . "'";
            //$json .= ",'" . addslashes($un->fil_nrocaj) . "'";
            ...
            $from = "FROM
                    ...
                    tab_exparchivo.exa_estado = 1 AND
                    tab_expusuario.eus_estado = 1 AND
                    tab_expusuario.usu_id='$usu_id' AND
                    tab_expediente.exp_id = '$un2->exp_id' ";
