 ...
 * @access public
 */
class buscarArchivoEnviadoController extends baseController {
    ...
    function index() {
        ...
        // REVISED CASTELLON
        $this->registry->template->cuerpos = $empresas->obtenerSelect();
        //$this->registry->template->cuerpos = $tramite->obtenerSelectCuerpos("");
        $this->registry->template->lugar = $usuario->obtenerSelect();
        ...
        $tmenu = new menu ();
        $liMenu = $tmenu->imprimirMenu("buscarArchivoEnviado", $_SESSION ['USU_ID']);
        ...
        $this->registry->template->show('buscarArchivoEnviado.tpl');
    ...
    function search() { //print("ingreso a search");die;
        ...
        $resultado = $arc->buscarEnviado($envia);
    ...
    function imprimirPdf() {
        ...
        $fil_descripcion = $_REQUEST['fil_descripcion']; //print $fil_descripcion;die;
        $institucion = $_REQUEST['institucion'];
        $lugar = $_REQUEST['lugar'];
        $fi = "";
        $ff = "";
        $ser_id = 2; // 1= correspondencia recibida 2= correspondencia enviada
        ...
        $select = "SELECT DISTINCT ta.fil_id, ta.fil_nomoriginal, (ta.fil_tamano/1048576) AS fil_tamano, ta.fil_tipo, ta.fil_descripcion, ta.fil_confidencialidad, te.exp_id, te.exp_nombre, te.exp_codigo, te.exp_descripcion, ts.ser_categoria, tt.tra_codigo, tt.tra_descripcion, tc.cue_codigo, tc.cue_descripcion, (CASE tea.exa_condicion WHEN '1' THEN 'DISPONIBLE' WHEN '2' THEN 'PRESTADO' END) AS disponibilidad, tin.ins_nombre, tf.fon_descripcion, (SELECT (ttipo.ctp_codigo || ' ' || ttc.con_codigo) as contenedor FROM tab_contenedor ttc Inner Join tab_exparchivo AS ttea ON ttea.con_id = ttc.con_id Inner Join tab_tipocontenedor AS ttipo ON ttc.ctp_id = ttipo.ctp_id WHERE ttea.fil_id=ta.fil_id) as contenedor,
            (SELECT (tipo.ctp_codigo || ' ' || ts.suc_nro) as subcontenedor FROM tab_exparchivo AS ea Inner Join tab_subcontenedor AS ts ON ea.suc_id = ts.suc_id Inner Join tab_tipocontenedor AS tipo ON ts.ctp_id = tipo.ctp_id WHERE ea.fil_id=ta.fil_id) as subcontenedor, te.emp_id, te.usu_env_id, te.exp_fecha_exi, te.emp_ext_id ";
        $from = "FROM
        ...
        if (strlen($tra_id) > 0) {
            $where .= " AND te.emp_ext_id='$tra_id' ";
        }
        if (strlen($cue_id) > 0) {
            $where .= " AND te.emp_id='$cue_id' ";
        }
        if (strlen($exf_fecha_exi) > 0) {
            $where .= " AND te.exp_fecha_exi>='$exf_fecha_exi' ";
            $fi = date("d/m/Y", strtotime($exf_fecha_exi));
        }
        if (strlen($exf_fecha_exf) > 0) {
            $where .= " AND te.exp_fecha_exi<='$exf_fecha_exf' ";
...
//        }
        ...
        $pdf->SetSubject('Filtro de Correspondencia Enviada');
        ...
        $pdf->Cell(80, 10, 'CORRESPONDENCIA ENVIADA', 0, 0, 'L', 0, '', 0);
        ...
        if (strlen($tra_id) > 0) {
            ...
            $cadena .= '<tr colspan="5"><td width="450"><b>DESTINATARIO : </b>' . $empresa_ext->obtenerNombre($result[0]->emp_ext_id) . '</td></tr>
            <tr align="center">
        <td width="20"><b>Nro.</b></td>
        <td width="80"><b>C&Oacute;DIGO</b></td>
        <td width="100"><b>ORIGEN</b></td>
        <td width="80"><b>USUARIO EMISI&Oacute;N</b></td>
        <td width="80"><b>EMISI&Oacute;N</b></td>
        <td width="90"><b>REFERENCIA</b></td>
                    </tr>';
            foreach ($result as $un) {
                $cadena .='<tr>
        <td width="20" align="right">&nbsp;&nbsp;' . $contador . 'nbsp;&nbsp;</td>
        <td width="80">&nbsp;&nbsp;' . $un->exp_codigo . '</td>
        <td width="100">&nbsp;&nbsp;' . $empresa->obtenerNombre($un->emp_id) . '</td>
        <td width="80">&nbsp;&nbsp;' . $usuario->obtenerNombre($un->usu_env_id) . '</td>
        <td width="80" align="center">&nbsp;&nbsp;' . date("d/m/Y", strtotime($un->exp_fecha_exi)) . '</td>
        <td width="90">&nbsp;&nbsp;' . $un->exp_descripcion . '</td>
        </tr>';
        ...
        <td width="80"><b>USUARIO EMISI&Oacute;N</b></td>
        <td width="80"><b>EMISI&Oacute;N</b></td>
        <td width="80"><b>DESTINATARIO</b></td>
        <td width="90"><b>REFERENCIA</b></td>
                    </tr>';
            foreach ($result as $un) {
                $cadena .='<tr>
        ...
        <td width="80">&nbsp;&nbsp;' . $usuario->obtenerNombre($un->usu_env_id) . '</td>
        <td width="80" align="center">&nbsp;&nbsp;' . date("d/m/Y", strtotime($un->exp_fecha_exi)) . '</td>
        <td width="80">&nbsp;&nbsp;' . $empresa_ext->obtenerNombre($un->emp_ext_id) . '</td>
        <td width="90">&nbsp;&nbsp;' . $un->exp_descripcion . '</td>
        </tr>';
