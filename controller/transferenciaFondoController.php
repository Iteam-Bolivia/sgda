...
/**
 * transferenciaFondoController.php Controller
 *
 * @package
 ...
 * @access public
 */
class transferenciaFondoController extends baseController {

    function index() {
        $fon_orden = 0; //FONDOS inicial
        $fon_orden_des = 1; // Fondo destinatario a la que se debe transferir
        $tab_fondos = new tab_fondo();
        $detFondo = $tab_fondos->dbselectByField("fon_orden", 1);
        $detFondo = $detFondo[0];
        //$detFondo = $fondos->obtenerFondo($_SESSION['USU_ID'], $fon_orden);
        ...
        $this->registry->template->tituloGeneral = "Transferencias a Fondo";
        ...
        if ($detFondo != null) {
            $this->registry->template->PATH_B = $this->series->loadMenu($adm, "test"); //$this->series->loadMenuTransfer($detFondo->inl_id, $adm, "test2" );
//            $this->registry->template->inst_fondo_id = $detFondo->inl_id;
            $this->registry->template->fondo_des = $detFondo->fon_descripcion;
            $this->registry->template->trn_usuario_des = $this->usuario->listUsuariosFondo($_SESSION ['USU_ID'], "SUBF", $fon_orden_des);
        } else {
            $fon_orden = '0'; //fondo
            $tinl = new institucion();
            $ins_fon = $tinl->obtenerSinUSR($fon_orden);
            $this->registry->template->PATH_B = $this->series->loadMenu($adm, "test"); //$this->series->loadMenuTransfer($ins_fon->inl_id, $adm, "test2" );
//            $this->registry->template->inst_fondo_id = $ins_fon->inl_id;
        ...
        $this->registry->template->show('transferenciaFondog.tpl');
    ...
    function loadExp() {
        ...
        $select = "SELECT DISTINCT te.exp_id, te.exp_nombre, te.exp_descripcion,te.exp_codigo, ts.ser_categoria, te.exp_fecha_exi,
            (te.exp_fecha_exi + ret.ret_anios * INTERVAL '1 year')::DATE as exp_fecha_exf ";
        if ($tipo == 'ADM') {
            $from = " FROM tab_expediente te
                ...
                INNER JOIN tab_retdocumental ret ON ts.ser_id=ret.ser_id AND ret.fon_id=1
                WHERE eu.eus_estado =  '1' AND te.exp_estado =  '1' AND ef.exf_estado =  '1' AND tu.usu_estado =  '1' AND ts.ser_estado =  '1' ";
        } else {
...
//            //$where .= " AND un.ins_id = '$ins_fondo->ins_id' ";
            ...
            if ($tipo == 'SUBF') {
                ...
                $where .= " AND (te.exp_fecha_exi + ret.ret_anios * INTERVAL '1 year')::DATE<=CURRENT_DATE ";
            } else {
                $where .= " AND ef.fon_id='1' ";
                $where .= " AND (te.exp_fecha_exi + ret.ret_anios * INTERVAL '1 year')::DATE<=CURRENT_DATE ";
            }
            if ($tipo == 'APY') {
                $where .= " AND us2.usu_id='" . $_SESSION['USU_ID'] . "' ";
            }
            //$from =" FROM tab_expediente te INNER JOIN tab_expfondo ef ON ef.exp_id=te.exp_id INNER JOIN tab_series ts ON te.ser_id=ts.ser_id INNER JOIN tab_expusuario eu ON eu.exp_id=te.exp_id INNER JOIN tab_usuario tu ON tu.usu_id=eu.usu_id INNER JOIN tab_unidad un ON tu.uni_id=un.uni_id INNER JOIN tab_usu_serie us ON us.usu_id = tu.usu_id AND us.ser_id=ts.ser_id INNER JOIN tab_usu_serie us2 ON us2.ser_id=ts.ser_id WHERE eu.eus_estado = '1' AND te.exp_estado = '1' AND ef.exf_estado = '1' AND tu.usu_estado = '1' AND ts.ser_estado = '1' AND us.use_estado = '1' AND us2.use_estado = '1' ";
        }


        /*         * *************** */

        //$sql = "$select $from $where $sort $limit";//$limit
        $sql = "$select $from $where $sort $limit";
        //$sql = "$select $from $sort $limit";//$limit
        $sql_c = "SELECT COUNT(DISTINCT te.exp_id) AS num $from $where $limit";
        $texp = new expediente (); //print $sql;die;
        $total = $texp->countExpATransf($where, '1');
    ...
    function loadFondo() {
        ...
        $tipo = $_SESSION['ROL_COD'];
        $fondo = new fondo();
        $fon_orden = '1'; // fon_orden = 1 para indicar que esta en fondo
        $inst_fondo = $fondo->obtenerFondo($_SESSION ['USU_ID'], $fon_orden);
...
SELECT t2.usu_id FROM tab_usuario t2 WHERE 1 ";
            ...
            elseif ($qtype == 'custodio') {
                ...
                $nomArray = explode(" ", $query);
                $where .= " and tab_transferencia.trn_usuario_des IN (SELECT t4.usu_id FROM tab_usuario t4 WHERE 1 ";
                foreach ($nomArray as $nom) {
...
//        }
        ...
        /* if($tipo == 'SUBF') {
          ...
          $where .= " AND ef.fon_id='3' ";
            ...
            $where .= " AND tab_transferencia.trn_usuario_orig ='" . $_SESSION['USU_ID'] . "'  AND us.usu_id='" . $_SESSION['USU_ID'] . "' ";
            //$where .= " AND ef.fon_id='1' ";
        }/* else {
          $where .= " AND ef.fon_id='1' ";
          } */
        ...
        /*         * *************** */
        $select = "SELECT DISTINCT tab_transferencia.trn_descripcion,tab_transferencia.trn_uni_origen,tab_transferencia.trn_uni_destino,tab_transferencia.trn_usuario_orig,tab_transferencia.trn_usuario_des,tab_series.ser_categoria,tab_transferencia.trn_id,te.exp_nombre,te.exp_descripcion,te.exp_codigo,tab_transferencia.exp_id,te.exp_fecha_exf,ef.exf_fecha_exf ";
        $from = "FROM tab_transferencia Inner Join tab_expediente te ON tab_transferencia.exp_id = te.exp_id Inner Join tab_series ON te.ser_id = tab_series.ser_id Inner Join tab_expfondo ef ON te.exp_id = ef.exp_id INNER JOIN tab_expusuario eu ON eu.exp_id=te.exp_id INNER JOIN tab_usu_serie us ON us.usu_id=eu.usu_id INNER JOIN tab_usuario u ON u.usu_id=us.usu_id INNER JOIN tab_unidad un ON un.uni_id=u.uni_id INNER JOIN tab_usu_serie us2 ON us2.ser_id=tab_series.ser_id WHERE trn_estado = 1 AND tab_transferencia.trn_usuario_orig=u.usu_id AND us.ser_id = tab_series.ser_id AND eu.eus_estado = '1' AND te.exp_estado = '1' AND ef.exf_estado = '1' AND us.use_estado = '1' AND us2.use_estado = '1'";
        //$sql = "$select $from $where $sort ";//$limit
        $sql = "$select $from $sort $limit"; //$limit
        $sql_c = "SELECT count( DISTINCT tab_transferencia.trn_id) as num $from $where";

        //print $sql;
        $result = $transf->dbselectBySQL($sql);
        $total = $transf->countBySQL($sql_c);
        ...
        foreach ($result as $un) {
            ...
            $json .= ",'" . addslashes($uni->getCodigo($un->trn_uni_origen)) . "'";
            $json .= ",'" . addslashes($usuario->obtenerNombre($un->trn_usuario_orig)) . "'";
            $json .= ",'" . addslashes($uni->getCodigo($un->trn_uni_destino)) . "'";
            $json .= ",'" . addslashes($usuario->obtenerNombre($un->trn_usuario_des)) . "'";
    ...
    function find() {
        ...
        if ($ids != '') {
            ...
            WHERE
            teus.exp_id IN($ids) AND teus.eus_estado = 1 ";
            $rowsUn = $unidad->dbselectBySQL($sqlUn);
            if (count($rowsUn) == 1) {
            ...
            foreach ($rowE as $exp) {
                ...
                $expeds[$i]['exp_nombre'] = $exp->exp_nombre;
                $i++;
    ...
    function save() {
        ...
        foreach ($expArray as $exp_id) {
            ...
            $tr->setTrn_confirmado('2'); /* colocando en esta secciÃƒÂ³n el fondo al q se envia inst_fondo */
